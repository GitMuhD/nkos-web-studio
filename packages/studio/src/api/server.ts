...
import { StudioIndexDB } from "./studio-index-db.js";
...
function sanitizeFsPath(rawPath: string): string {
  return rawPath.replace(/[\u202a-\u202e\u2066-\u2069\ufeff]/g, "").trim().replace(/^['\"]|['\"]$/g, "");
}

function defaultBrowseRoots(projectRoot: string): string[] {
  const home = homedir();
  const candidates = [
    join(home, "Desktop"),
    join(home, "Downloads"),
...
function isAllowedBrowsePath(path: string, projectRoot: string): boolean {
  const resolvedPath = resolve(path);
  return defaultBrowseRoots(projectRoot).some((rootPath) => {
    const rootResolved = resolve(rootPath);
    return resolvedPath === rootResolved || resolvedPath.startsWith(rootResolved + "\\") || resolvedPath.startsWith(rootResolved + "/");
  });
}

function isChapterFileName(fileName: string): boolean {
  return /第\s*\d{1,5}\s*章/.test(fileName) || /chapter\s*\d{1,5}/i.test(fileName);
}

function extractChapterNumberFromName(fileName: string, fallback: number): number {
  const zh = fileName.match(/第\s*(\d{1,5})\s*章/);
  if (zh) return Number(zh[1]);
  const en = fileName.match(/chapter\s*(\d{1,5})/i);
  if (en) return Number(en[1]);
  const nums = fileName.match(/\d+/g);
  return nums ? Number(nums.at(-1)) : fallback;
}

function buildChapterHeading(fileName: string, chapterNumber: number): string {
  const base = fileName.replace(/\.(md|txt)$/i, "");
  if (/第\s*\d{1,5}\s*章/.test(base) || /chapter\s*\d{1,5}/i.test(base)) {
    return base;
  }
  return `第${chapterNumber}章 ${base}`;
}
...
function sanitizeChapterFileName(title: string): string {
  const cleaned = title
    .replace(/[/\\?%*:|"<>]/g, "")
    .replace(/\s+/g, "_")
    .trim()
...
function countImportedChapterWords(content: string, language?: string): number {
  if (language === "en") {
    return content.trim().split(/\s+/).filter(Boolean).length;
  }
  return (content.match(/[\u4e00-\u9fff]|[A-Za-z0-9]+/g) ?? []).length;
}

function splitFolderImportedChapters(text: string): ReadonlyArray<{ title: string; content: string }> | null {
  const markerRegex = /^<!-- INKOS_CHAPTER_FILE:(.*?) -->\s*$/gm;
  ...
  return markers.map((marker, index) => {
    ...
    const raw = text.slice(markerEnd, nextStart).trim();
    const lines = raw.split("\n");
    const firstLine = lines[0]?.replace(/^#+\s*/, "").trim() ?? "";
    const fallbackName = marker[1]?.replace(/\.(md|txt)$/i, "") ?? `第${index + 1}章`;
    const title = firstLine || fallbackName;
    const content = lines.slice(firstLine ? 1 : 0).join("\n").trim();
    return { title, content };
  }).filter((chapter) => chapter.content.trim().length > 0);
}

async function readTextFileWithFallback(filePath: string): Promise<string> {
...
function updateAgentTaskList(
  ...
  const tasks = task.tasks.map((item) => {
    ...
    if (!seenActive && item.status !== "error" && item.status !== "stopped") {
...
function updateAgentStatus(
  sessionId: string,
  patch: Partial<Omit<RunningAgentTask, "sessionId" | "startedAt" | "instruction">> & { activeTaskId?: string; taskStatus?: AgentTaskItem["status"] },
): void {
  const current = runningAgentSessions.get(sessionId);
  if (!current) return;
  const { activeTaskId, taskStatus, ...statusPatch } = patch;
  let next = { ...current, ...statusPatch };
  next = updateAgentTaskList(next, activeTaskId, taskStatus, next.detail);
  next.completedTasks = next.tasks.filter((task) => task.status === "completed").length;
  next.totalTasks = next.tasks.length;
  next.waitingTasks = next.tasks.filter((task) => task.status === "pending").length;
  runningAgentSessions.set(sessionId, next);
  broadcast("agent:status", {
    sessionId,
    activeBookId: next.activeBookId,
    instruction: next.instruction,
    currentTask: next.currentTask,
    stage: next.stage,
    completedTasks: next.completedTasks,
    totalTasks: next.totalTasks,
    waitingTasks: next.waitingTasks,
    detail: next.detail,
    tasks: next.tasks,
...
function throwIfSessionStopped(sessionId: string): void {
  if (stopRequestedSessions.has(sessionId)) {
    throw new ApiError(499, "AGENT_STOPPED", "当前任务已停止");
  }
}
...
const bookImportLocks = new Set<string>();
type AgentTaskItem = {
  id: string;
  title: string;
  description?: string;
...
const runningAgentSessions = new Map<string, RunningAgentTask>();
...
function broadcastImportProgress(bookId: string, stage: string, message: string, extra: Record<string, unknown> = {}): void {
  const data = { bookId, type: "existing-book", stage, message, ...extra };
  broadcast("import:progress", data);
  broadcast("log", { level: "info", message: `[导入] ${message}`, bookId, stage });
}
...
export function createStudioServer(initialConfig: ProjectConfig, root: string) {
  ...
  const indexDbPromise = StudioIndexDB.open(root).catch((error) => {
    console.warn("[studio-index] SQLite index unavailable; falling back to file storage:", error instanceof Error ? error.message : String(error));
    return null;
  });
  let cachedConfig = initialConfig;

  async function getIndexDb(): Promise<StudioIndexDB | null> {
    return indexDbPromise;
  }

  async function syncBookToIndex(bookId: string): Promise<void> {
    const db = await getIndexDb();
    if (!db) return;
    const book = await state.loadBookConfig(bookId);
    const chapters = await state.loadChapterIndex(bookId);
    db.upsertBookWithChapters(book, chapters);
  }

  async function listIndexedOrFileBooks() {
    const db = await getIndexDb();
    if (db) {
      const indexedBooks = db.listBooks();
      if (indexedBooks.length > 0) return indexedBooks;
    }
    ...
    const books = await Promise.all(
      bookIds.map(async (id) => {
        ...
        if (db) db.upsertBookWithChapters(book, chapters);
        return {
          ...book,
  ...
  // --- Local folder import helpers ---

  app.get("/api/v1/fs/roots", (c) => {
    const roots = defaultBrowseRoots(root).map((path) => ({
  ...
  app.post("/api/v1/fs/list", async (c) => {
    const body = await c.req.json<{ path?: string }>().catch((): { path?: string } => ({}));
    const roots = defaultBrowseRoots(root);
    const current = resolve(sanitizeFsPath(body.path || roots[0] || root));
    if (!isAllowedBrowsePath(current, root)) {
      throw new ApiError(403, "FS_PATH_FORBIDDEN", "Only Desktop, Downloads, Documents and current project folders can be browsed.");
    }
    ...
    return c.json({
      path: current,
      parent: parent !== current && isAllowedBrowsePath(parent, root) ? parent : null,
      directories,
    });
  });

  app.post("/api/v1/import/read-folder", async (c) => {
    const body = await c.req.json<{ path: string }>();
    const folder = resolve(sanitizeFsPath(body.path));
    if (!isAllowedBrowsePath(folder, root)) {
      throw new ApiError(403, "FS_PATH_FORBIDDEN", "Only Desktop, Downloads, Documents and current project folders can be imported.");
    }
    const entries = await readdir(folder, { withFileTypes: true });
    const files = entries
      .filter((entry) => entry.isFile() && /\.(md|txt)$/i.test(entry.name) && isChapterFileName(entry.name))
      .map((entry, index) => ({
        name: entry.name,
        path: join(folder, entry.name),
        chapterNumber: extractChapterNumberFromName(entry.name, index + 1),
      }))
      .sort((a, b) => a.chapterNumber - b.chapterNumber || a.name.localeCompare(b.name, "zh-CN"));

    const parts: string[] = [];
    const chapterSummaries: Array<{ name: string; chapterNumber: number; title: string; wordCount: number }> = [];
    let totalWords = 0;
    for (const file of files) {
      const content = (await readTextFileWithFallback(file.path)).trim();
      if (!content) continue;
      const heading = buildChapterHeading(file.name, file.chapterNumber);
      const normalizedContent = content.startsWith(heading) ? content : `${heading}\n\n${content}`;
      const lines = normalizedContent.split("\n");
      const title = lines[0]?.replace(/^#+\s*/, "").trim() || heading;
      const body = lines.slice(1).join("\n").trim();
      const wordCount = countImportedChapterWords(body || normalizedContent, "zh");
      ...
      parts.push(`<!-- INKOS_CHAPTER_FILE:${file.name.replace(/--/g, "-")} -->\n${normalizedContent}`);
    }
  ...
  app.get("/api/v1/books", async (c) => {
    const books = await listIndexedOrFileBooks();
    return c.json({ books });
  });
  ...
  app.get("/api/v1/books/:id", async (c) => {
    ...
    try {
      const db = await getIndexDb();
      const indexedBook = db?.getBook(id);
      if (indexedBook) {
        const chapters = db!.listChapters(id);
        return c.json({ book: indexedBook, chapters, nextChapter: chapters.length + 1 });
      }
      const book = await state.loadBookConfig(id);
      const chapters = await state.loadChapterIndex(id);
      if (db) db.upsertBookWithChapters(book, chapters);
      return c.json({ book, chapters, nextChapter: chapters.length + 1 });
    } catch {
  ...
  app.post("/api/v1/books/create", async (c) => {
    ...
    const bookId = slugifyBookId(bookConfig.id || body.title);
  ...
  app.post("/api/v1/books/import-existing", async (c) => {
    const body = await c.req.json<{
      title: string;
      genre: string;
      language?: string;
    ...
    const bookConfig = buildStudioBookConfig(body, now);
    const bookId = slugifyBookId(bookConfig.id || body.title);
    const finalBookConfig = { ...bookConfig, id: bookId };
    const bookDir = state.bookDir(bookId);

    if (bookImportLocks.has(bookId)) {
      return c.json({ error: `Book "${bookId}" is already importing` }, 409);
    }
    bookImportLocks.add(bookId);

    try {
      await access(join(bookDir, "book.json"));
      const existingChapters = await state.loadChapterIndex(bookId);
      if (existingChapters.length > 0) {
        await syncBookToIndex(bookId).catch(() => undefined);
        return c.json({
          bookId,
          alreadyExists: true,
          importedCount: existingChapters.length,
          totalWords: existingChapters.reduce((sum, chapter) => sum + (chapter.wordCount ?? 0), 0),
          nextChapter: existingChapters.length + 1,
        });
      }
      return c.json({ error: `Book "${bookId}" already exists, but has no imported chapters. Delete it before importing again.` }, 409);
    } catch {
      // ok
    }

    broadcast("import:start", { bookId, type: "existing-book", stage: "start", message: "开始导入已有小说" });
    try {
      broadcastImportProgress(bookId, "book", "正在创建书籍项目");
      await state.saveBookConfig(bookId, finalBookConfig as any);
      await syncBookToIndex(bookId).catch(() => undefined);
      await state.ensureControlDocuments(bookId, body.text.slice(0, 4000));
      const storyDir = join(bookDir, "story");
      await mkdir(storyDir, { recursive: true });
      const seedFiles: Record<string, string> = {
        "story_bible.md": `# ${body.title}\n\n（由已有小说导入，后续可在 Studio 中完善世界观、人物、规则和长期方向。）\n`,
      ...
      broadcastImportProgress(bookId, "truth", "正在初始化真相文件");
      await Promise.all(Object.entries(seedFiles).map(async ([fileName, content]) => {
        await writeFile(join(storyDir, fileName), content, "utf-8");
      }));
      await state.saveChapterIndex(bookId, []);

      broadcastImportProgress(bookId, "split", "正在切分章节");
      const { splitChapters } = await import("@actalk/inkos-core");
      const chapters = [...(splitFolderImportedChapters(body.text) ?? splitChapters(body.text, body.splitRegex))];
      if (chapters.length === 0) {
        throw new Error("没有识别到章节。请检查章节标题格式，或填写章节切分正则。");
      }
      broadcastImportProgress(bookId, "chapters", `已识别 ${chapters.length} 个章节，正在写入书籍`, { count: chapters.length });
      const chaptersDir = join(bookDir, "chapters");
      await mkdir(chaptersDir, { recursive: true });
      const nowForChapters = new Date().toISOString();
      let totalWords = 0;
      const chapterIndex = chapters.map((chapter, index) => {
        const chapterNumber = index + 1;
        const wordCount = countImportedChapterWords(chapter.content, body.language);
        totalWords += wordCount;
        return {
          ...
          wordCount,
          createdAt: nowForChapters,
          updatedAt: nowForChapters,
          auditIssues: [],
          lengthWarnings: [],
        };
      });
      for (const chapter of chapterIndex) {
        const source = chapters[chapter.number - 1]!;
        const paddedNum = String(chapter.number).padStart(4, "0");
        const heading = body.language === "en"
          ? `# Chapter ${chapter.number}: ${chapter.title}`
          : `# 第${chapter.number}章 ${chapter.title}`;
        await writeFile(
          join(chaptersDir, `${paddedNum}_${sanitizeChapterFileName(chapter.title)}.md`),
          `${heading}\n\n${source.content.trim()}\n`,
          "utf-8",
        );
      }
      await state.saveChapterIndex(bookId, chapterIndex);
      await syncBookToIndex(bookId);
      const verifiedIndex = await state.loadChapterIndex(bookId);
      if (verifiedIndex.length !== chapterIndex.length) {
        throw new Error(`章节写入校验失败：预期 ${chapterIndex.length} 章，实际 ${verifiedIndex.length} 章。`);
      }
      const result = { bookId, importedCount: chapterIndex.length, totalWords, nextChapter: chapterIndex.length + 1 };
      broadcast("import:complete", { bookId, type: "existing-book", stage: "complete", count: result.importedCount, message: `导入完成：${result.importedCount} 章` });
      return c.json({ ...result, bookId });
    } catch (e) {
      const errorMessage = e instanceof Error ? e.stack ?? e.message : String(e);
      broadcast("import:error", { bookId, error: errorMessage });
      broadcast("log", { level: "error", message: `[导入失败] ${errorMessage}`, bookId, stage: "error" });
      return c.json({ error: errorMessage }, 500);
    } finally {
      bookImportLocks.delete(bookId);
    }
  });
  ...
  app.post("/api/v1/books/:id/write-next", async (c) => {
    ...
    pipeline.writeNextChapter(id, body.wordCount).then(
      async (result) => {
        await syncBookToIndex(id).catch(() => undefined);
  ...
  app.post("/api/v1/agent/stop", async (c) => {
    const { sessionId } = await c.req.json<{ sessionId?: string }>().catch(() => ({}));
    if (!sessionId?.trim()) {
      throw new ApiError(400, "SESSION_ID_REQUIRED", "sessionId is required");
    }
    const running = runningAgentSessions.get(sessionId);
    stopRequestedSessions.add(sessionId);
    broadcast("agent:stopped", { sessionId, activeBookId: running?.activeBookId, instruction: running?.instruction });
    return c.json({ ok: true, stopped: Boolean(running), sessionId });
  });
  ...
  app.post("/api/v1/agent", async (c) => {
    ...
    runningAgentSessions.set(sessionId, {
      sessionId,
      startedAt: Date.now(),
      instruction,
      ...
      totalTasks: 5,
      waitingTasks: 4,
      detail: "正在读取模型配置、会话上下文和书籍状态。",
      tasks: [
        { id: "context", title: "加载模型与会话", description: "读取项目配置、模型服务、当前聊天会话和书籍状态。", status: "pending" },
        { id: "model", title: "选择可用模型", description: "根据前端选择和项目配置解析实际调用的模型。", status: "pending" },
        { id: "pipeline", title: "创建执行管线", description: "准备可调用工具、书籍上下文和多 Agent 执行管线。", status: "pending" },
        { id: "agent", title: "执行用户指令", description: "模型分析指令，必要时调用写作、审计、修订或文件工具。", status: "pending" },
        { id: "save", title: "整理并保存结果", description: "整理回复、保存会话记录并刷新界面状态。", status: "pending" },
      ],
    });
    broadcast("agent:start", { instruction, activeBookId, sessionId });
    updateAgentStatus(sessionId, {});

    try {
      updateAgentStatus(sessionId, {
        currentTask: "加载模型与会话",
        stage: "准备上下文",
        activeTaskId: "context",
        detail: "正在读取项目配置、模型服务和当前聊天会话。",
      });
      throwIfSessionStopped(sessionId);
      ...
      updateAgentStatus(sessionId, {
        currentTask: "创建执行管线",
        stage: "任务编排",
        activeTaskId: "pipeline",
        detail: "正在准备可调用工具、书籍上下文和多 Agent 执行管线。",
      });
      ...
      updateAgentStatus(sessionId, {
        ...
        stage: "Agent 运行中",
        activeTaskId: "agent",
        detail: "模型正在分析指令，必要时会调用写作、审计、修订或文件工具。",
      });
      throwIfSessionStopped(sessionId);
      ...
      const result = await runAgentSession(
        {
          ...
          onEvent: (event) => {
            ...
            if (event.type === "tool_execution_start") {
              ...
              updateAgentStatus(sessionId, {
                currentTask: resolveToolLabel(event.toolName, agent),
                stage: agent ? "子 Agent 执行" : "工具调用",
                activeTaskId: "agent",
                detail: agent
                  ? `正在运行 ${resolveToolLabel(event.toolName, agent)}，包含 ${stages.length} 个内部阶段。`
            ...
            if (event.type === "tool_execution_end") {
              ...
              updateAgentStatus(sessionId, {
                currentTask: event.isError ? "工具执行失败" : "工具执行完成",
                stage: event.isError ? "等待处理错误" : "继续编排下一步",
                activeTaskId: "agent",
                taskStatus: event.isError ? "error" : "running",
                detail: event.isError ? extractToolError(event.result) : `${resolveToolLabel(event.toolName, exec?.agent)} 已完成，正在判断下一步。`,
              });
      ...
      runningAgentSessions.delete(sessionId);
      stopRequestedSessions.delete(sessionId);
    }
  });
