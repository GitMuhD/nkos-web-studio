...
export const createMessageSlice: StateCreator<ChatStore, [], [], MessageActions> = (set, get) => ({
  ...
  stopMessage: async (sessionId) => {
    ...
    set((state) => ({
      sessions: updateSession(state.sessions, sessionId, (runtime) => ({
        ...
        messages: [
          ...runtime.messages,
          { role: "assistant", content: "已停止当前指令。", timestamp: Date.now() },
        ],
      })),
    }));
  },
  ...
  sendMessage: async (sessionId, text, activeBookId) => {
    ...
    set((state) => ({
      sessions: updateSession(state.sessions, sessionId, () => ({ stream: streamEs, requestController })),
    }));
