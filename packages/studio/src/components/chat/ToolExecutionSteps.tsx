...
export function ToolExecutionSteps({ executions, outputFontSize = 14 }: ToolExecutionStepsProps) {
  const groups = useMemo(() => groupChronologically(executions), [executions]);

  return (
    <div className="space-y-2 mt-2" style={{ fontSize: `${Math.max(11, outputFontSize - 2)}px` }}>
      {groups.map((g, i) =>
        g.type === "pipeline"
