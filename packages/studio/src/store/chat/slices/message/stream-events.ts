...
export function attachSessionStreamListeners({
  ...
  streamEs.addEventListener("agent:status", (event: MessageEvent) => {
    try {
      const data = event.data ? JSON.parse(event.data) : null;
      if (!sessionMatchesEvent(sessionId, data)) return;
      set((state) => ({
        sessions: updateSession(state.sessions, sessionId, () => ({
          taskStatus: {
            currentTask: String(data.currentTask ?? "执行中"),
            stage: String(data.stage ?? "运行中"),
            completedTasks: Number(data.completedTasks ?? 0),
            totalTasks: Number(data.totalTasks ?? 1),
            waitingTasks: Number(data.waitingTasks ?? 0),
            detail: typeof data.detail === "string" ? data.detail : undefined,
            tasks: Array.isArray(data.tasks)
              ? data.tasks.map((task: any) => ({
                  id: String(task.id ?? task.title ?? Math.random()),
                  title: String(task.title ?? "未命名任务"),
                  description: typeof task.description === "string" ? task.description : undefined,
                  status: ["pending", "running", "completed", "error", "stopped"].includes(task.status) ? task.status : "pending",
                }))
              : undefined,
  ...
  streamEs.addEventListener("agent:complete", (event: MessageEvent) => {
    try {
      ...
      if (!sessionMatchesEvent(sessionId, data)) return;
      set((state) => ({
        sessions: updateSession(state.sessions, sessionId, () => ({ taskStatus: null })),
      }));
    } catch {
      // ignore
    }
