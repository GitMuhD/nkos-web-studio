...
const MAX_TRUTH_CONTEXT_CHARS = 12_000;
const MAX_PRIORITY_FILE_CHARS = 3_000;
const MAX_SECONDARY_FILE_CHARS = 1_200;

function truncateTruthContent(name: string, content: string, maxChars: number): string {
  if (content.length <= maxChars) return content;
  return `${content.slice(0, maxChars)}\n\n[${name} 已截断：原长度 ${content.length} 字符，仅注入前 ${maxChars} 字符。如需全文，请使用 read 工具读取该文件。]`;
}
...
async function readTruthFiles(storyDir: string): Promise<TruthFileSection[]> {
  ...
  for (const fileName of ordered) {
    try {
      const raw = await readFile(join(storyDir, fileName), "utf-8");
      const isPriority = prioritySet.has(fileName);
      const maxForFile = isPriority ? MAX_PRIORITY_FILE_CHARS : MAX_SECONDARY_FILE_CHARS;
      const remaining = MAX_TRUTH_CONTEXT_CHARS - totalChars;
      if (remaining <= 0) break;
      const content = truncateTruthContent(fileName, raw, Math.min(maxForFile, remaining));
